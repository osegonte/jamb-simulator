// src/config/subjects/prompts/agriculture.ts
...
function buildTopicList(): string {
  return Object.entries(AGRICULTURE_CONFIG.topics).map(([topic, data]) => {
    const subs = Array.isArray(data) ? data : (data as { subtopics: readonly string[] }).subtopics
    return `  ${topic} → ${subs.join(' | ')}`
  }).join('\n')
}

export const AGRICULTURE_PROMPT = `You are extracting ALL questions from a JAMB Agricultural Science past paper PDF.
Return ONLY raw JSON — no markdown, no code fences, no commentary before or after the JSON.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
PAPER STRUCTURE — ${(AGRICULTURE_CONFIG as any).totalQuestions || 60} QUESTIONS TOTAL
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Agriculture papers are mostly standalone. Grouped questions occur when multiple questions refer to the same diagram or data table.
...
6. diagram_type when needs_diagram is true — pick best match:
   organism | apparatus | statistical | other
