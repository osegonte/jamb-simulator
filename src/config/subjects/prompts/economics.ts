// src/config/subjects/prompts/economics.ts
...
function buildTopicList(): string {
  return Object.entries(ECONOMICS_CONFIG.topics).map(([topic, data]) => {
    const subs = Array.isArray(data) ? data : (data as { subtopics: readonly string[] }).subtopics
    return `  ${topic} → ${subs.join(' | ')}`
  }).join('\n')
}

export const ECONOMICS_PROMPT = `You are extracting ALL questions from a JAMB Economics past paper PDF.
Return ONLY raw JSON — no markdown, no code fences, no commentary before or after the JSON.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
PAPER STRUCTURE — ${(ECONOMICS_CONFIG as any).totalQuestions || 60} QUESTIONS TOTAL
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Economics papers often include graphs (supply/demand curves, production possibility curves). These are standalone unless multiple questions share one figure.
